...
var Formats = []patterns.Format{
	...
	{
		Name: "header_format",
		Pattern: `\d+\s+(?P<msg_type>\w+)\s+(?P<hdr1>\S+)\s+` +
			`(?:/\d+\s+)?(?P<origin>{ICAO})/(?P<dest>{ICAO})\s+\.?(?P<tail>\S+)`,
		Fields: []string{"msg_type", "hdr1", "origin", "dest", "tail"},
	},
	...
	{
		Name: "position",
		Pattern: `(?:/POS|\bPOS)\s+(?P<lat_dir>{LAT_DIR})(?P<lat>\d+\.?\d*)[/\s]*` +
	...
	// Report time extraction (TME).
	{
		Name:    "tme",
		Pattern: `(?:/)?TME\s+(?P<tme>\d{3,4})`,
		Fields:  []string{"tme"},
	},
	// Wind extraction: /WND -94 76
	{
		Name:    "wind",
		Pattern: `/WND\s+(?P<wdir>-?\d+)\s+(?P<wspd>\d+)`,
		Fields:  []string{"wdir", "wspd"},
	},
	// Outside air temperature extraction.
	{
		Name:    "oat",
		Pattern: `/OAT\s+(?P<oat>-?\d+)`,
		Fields:  []string{"oat"},
	},
	// OUT time extraction.
	{
