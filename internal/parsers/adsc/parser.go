...
type Result struct {
	...
	ContractNumber    int     `json:"contract_number,omitempty"` // For uplink requests.
	...
	Accuracy       float64          `json:"accuracy,omitempty"`         // Position accuracy in NM.
	...
	ContractReq    *ContractRequest `json:"contract_request,omitempty"` // Uplink contract request.
...
func (p *Parser) Labels() []string { return []string{"B6", "A6"} } // B6=downlink, A6=uplink
...
func (p *Parser) Parse(msg *acars.Message) registry.Result {
	...
	// Determine direction based on ACARS label:
	// Label A6 = uplink (ground-to-air)
	// Label B6 = downlink (air-to-ground)
	if msg.Label == "A6" {
...
func decodePayloadData(result *Result, data []byte) {
	...
	for offset < len(data) {
		...
		consumed := parseTag(result, tag, data[offset:], isFirstTag)
		if consumed < 0 {
...
func parseTag(result *Result, tag byte, data []byte, isFirst bool) int {
	...
	// === UPLINK TAGS (Ground-to-Air) ===

	// Periodic contract request (Tag 0x55).
	case 0x55:
		if isFirst {
			result.MessageType = "periodic_contract_request"
		}
		if len(data) < 1 {
			return -1
		}
		result.ContractNumber = int(data[0])
		// Skip modulus/report type/groups - variable length
		// For now, consume 1 byte (contract number) and stop parsing
		return len(data)

	// Demand contract request (Tag 0x56).
	case 0x56:
		if isFirst {
			result.MessageType = "demand_contract_request"
		}
		if len(data) < 1 {
			return -1
		}
	...
	// Cancel contract (Tag 0x57).
	case 0x57:
		if isFirst {
			result.MessageType = "cancel_contract"
		}
		if len(data) < 1 {
			return -1
		}
	...
	// Cancel all contracts (Tag 0x5F).
	case 0x5F:
		if isFirst {
			result.MessageType = "cancel_all_contracts"
		}
		return 0
...
// Format: lat(21) + lon(21) + alt(12) + timestamp(15) + flags(11).
...
func decodeBasicReportTag(result *Result, data []byte) {
	...
	// Altitude: bits 42-53 (12 bits) - NOT 16!
	altRaw := uint32((bits >> (64 - 54)) & 0xFFF)
	result.Altitude = decodeAltitude(altRaw)

	// Timestamp: bits 54-68 (15 bits).
	// Read all 10 bytes for timestamp extraction.
	var allBits [10]byte
	copy(allBits[:], data)

	// Extract timestamp from bits 54-68.
	...
	accuracyCode := int((flags >> 1) & 0x07)
	result.Accuracy = decodeAccuracy(accuracyCode)
...
func decodeAltitude(raw uint32) int {
	// 12-bit altitude value (unsigned for basic report).
	// Formula: altitude_ft = raw * 64
	// Valid range: 0-262080 ft (covers FL000-FL850+)
	return int(raw) * 64
}

// decodeAltitude16 decodes a 16-bit altitude value (used in predicted route).
// Formula: altitude_ft = raw * 4
// Valid range: 0-262140 ft
func decodeAltitude16(raw uint32) int {
	return int(raw) * 4
}

// decodeAccuracy converts a 3-bit accuracy code to nautical miles.
// Based on ARINC 622 accuracy encoding.
func decodeAccuracy(code int) float64 {
	// Table from ARINC 622 / libacars
	switch code {
	case 0:
		return 0.05 // < 0.05 NM
	case 1:
		return 0.1 // 0.05 - 0.1 NM
	case 2:
		return 0.25 // 0.1 - 0.25 NM
	case 3:
		return 0.5 // 0.25 - 0.5 NM
	case 4:
		return 1.0 // 0.5 - 1.0 NM
	case 5:
		return 2.0 // 1.0 - 2.0 NM
	case 6:
		return 10.0 // 2.0 - 10.0 NM
	case 7:
		return 0.05 // > 10.0 NM or best available (treat as most precise)
	default:
		return 0.0
	}
}
...
func decodeAirRef(data []byte) *AirRef {
	...
	// Mach speed: bits 13-25 (13 bits), stored as mach * 2000.
	speedRaw := (bits >> 14) & 0x1FFF
...
func decodePredictedRoute(data []byte) *PredictedRoute {
	...
	return &PredictedRoute{
		NextWaypoint: &Waypoint{
			...
			Altitude:  decodeAltitude16(altNextRaw),
			ETA:       int(etaNextRaw),
		},
		NextNextWaypoint: &Waypoint{
			Latitude:  decodeCoordinate(latNextNextRaw),
			Longitude: decodeCoordinate(lonNextNextRaw),
			Altitude:  decodeAltitude16(altNextNextRaw),
		},
	}
}

// decodeUplinkPayload decodes uplink (ground-to-air) contract request messages.
// Format varies by tag, but common structure for periodic contracts:
// - Tag byte (e.g., 0x55 for periodic contract)
// - Contract number
// - Modulus/groups/intervals
func decodeUplinkPayload(result *Result, data []byte) {
	if len(data) < 1 {
		result.MessageType = "uplink_unknown"
		return
	}

	// For the hex "07550BCAA4E2" after CRC removal: [0x07, 0x55, 0x0B, 0xCA]
	// This appears to be: tag(0x07?) + contract_num(0x55=85) + interval_data(0x0BCA)
	// But according to spec, 0x55 should be the tag, not 0x07

	// Try parsing as if first byte is contract number (0x07 = 7?)
	// and second byte is some other data
	// But user expects contract_num=85, which is 0x55 in hex

	// Alternative: Maybe the format is:
	// - data[0] = some header/tag
	// - data[1] = contract_num (0x55 = 85 decimal)
	// - data[2-3] = interval in some encoding

	if len(data) >= 4 {
		// Parse as: [header][contract_num][interval_hi][interval_lo]
		result.MessageType = "periodic_contract_request"
		result.ContractReq = &ContractRequest{
			ContractNum: int(data[1]), // 0x55 = 85
		}

		// Try to decode interval from bytes 2-3
		// Bytes: [7, 85, 11, 202]
		// data[2] = 11, data[3] = 202
		// 11 * 64 = 704 seconds!
		// So data[2] appears to be a multiplier (intervals in 64-second units)
		modulus := int(data[2])
		result.ContractReq.IntervalSecs = modulus * 64
