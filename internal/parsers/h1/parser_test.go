...
func TestH1PosParse_ExtendedFormat(t *testing.T) {
// Test extended H1 POS format with additional fields after wind data.
msg := &acars.Message{
ID:        2,
Label:     "H1",
Text:      "POSN45209E023245,INVED,105718,330,LUGEB,112508,UDROS,M59,33450,1808/TS105718,010126904E",
Timestamp: "2026-01-10T10:57:18Z",
Tail:      "TEST123",
}
...
// Check coordinates (N 45 20.9' = 45.348, E 023 24.5' = 23.408)
if abs(pos.Latitude-45.348333) > 0.001 {
t.Errorf("latitude = %v, want ~45.348", pos.Latitude)
}
if abs(pos.Longitude-23.408333) > 0.001 {
t.Errorf("longitude = %v, want ~23.408", pos.Longitude)
}

if pos.FlightLevel != 330 {
t.Errorf("flight level = %d, want 330", pos.FlightLevel)
}
if pos.Temperature != -59 {
t.Errorf("temperature = %d, want -59", pos.Temperature)
}
if pos.WindDir != 334 {
t.Errorf("wind_dir = %d, want 334", pos.WindDir)
}
if pos.WindSpeed != 50 {
t.Errorf("wind_speed = %d, want 50", pos.WindSpeed)
}
if pos.CurrentWaypoint != "INVED" {
t.Errorf("current_waypoint = %s, want INVED", pos.CurrentWaypoint)
}
if pos.NextWaypoint != "LUGEB" {
t.Errorf("next_waypoint = %s, want LUGEB", pos.NextWaypoint)
}
if pos.ThirdWaypoint != "UDROS" {
t.Errorf("third_waypoint = %s, want UDROS", pos.ThirdWaypoint)
}
if pos.ReportTime != "105718" {
t.Errorf("report_time = %s, want 105718", pos.ReportTime)
}
if pos.ETA != "112508" {
t.Errorf("eta = %s, want 112508", pos.ETA)
}
}
